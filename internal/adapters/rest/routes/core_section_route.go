...
func NewCoreSectionRoute(router fiber.Router, sectionService services.SectionService, labSectionService services.LabSectionService, labService services.LabService, sectionStudentService services.SectionStudentService, labMaterialService services.LabMaterialService, courseService services.CourseService) {
	...
	// GET /api/v1/core/sections - List enrolled sections (filtered by student_id in handler)
	...
	// GET /api/v1/core/sections/:sectionID - Must be enrolled student
	coreSectionRouter.Get("/:sectionID",
		middlewares.PermissionMiddleware(middlewares.IsSectionStudent(sectionStudentService, "sectionID")),
	...
	// GET /api/v1/core/sections/:sectionID/labs/:labID - Must be enrolled student
	coreSectionRouter.Get("/:sectionID/labs/:labID",
		middlewares.PermissionMiddleware(middlewares.IsSectionStudent(sectionStudentService, "sectionID")),
		func(c fiber.Ctx) error {
			sectionID := c.Params("sectionID")
	...
	// GET /api/v1/core/sections/:sectionID/labs - Must be enrolled student
	coreSectionRouter.Get("/:sectionID/labs",
		middlewares.PermissionMiddleware(middlewares.IsSectionStudent(sectionStudentService, "sectionID")),
		func(c fiber.Ctx) error {
			pageQuery := c.Query("page", "1")
			pageSizeQuery := c.Query("page_size", "10")
	...
	// DELETE /api/v1/core/sections/:sectionID/unenroll - Must be enrolled student
	coreSectionRouter.Delete("/:sectionID/unenroll",
		middlewares.PermissionMiddleware(middlewares.IsSectionStudent(sectionStudentService, "sectionID")),
		func(c fiber.Ctx) error {
			sectionID := c.Params("sectionID")
			user := c.Locals("user").(*models.User)
