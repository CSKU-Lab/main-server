...
func (s *courseService) GetPaginationForInstructor(ctx context.Context, instructorID string, page int, pageSize int, search string, sortBy string, sortOrder string, show string) ([]models.Course, error) {
	allowedSortFields := map[string]bool{
		"name":       true,
		"type":       true,
		"created_at": true,
	...
	courses, err := s.courseRepo.GetPaginationForInstructor(ctx, instructorID, page, pageSize, search, sanitizedSortBy, sanitizedSortOrder, show)
	if err != nil {
		return nil, err
	}
...
func (s *courseService) CountForInstructor(ctx context.Context, instructorID string, search string, show string) (int, error) {
	if show != "all" && show != "active" && show != "archived" {
		return 0, cserrors.New(&cserrors.Option{HttpStatus: http.StatusBadRequest, Message: "Invalid show value. Must be 'all', 'active', or 'archived'"})
	}

	return s.courseRepo.CountForInstructor(ctx, instructorID, search, show)
}
